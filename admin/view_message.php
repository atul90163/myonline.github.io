...
<textarea name="replymessage" rows="5" style="width:100%; background-color:#d9d9d9; padding:5px; border-bottom:2px solid red; " class="thover" placeholder="reply message...."  required></textarea>
 <center> <button type="submit" class="btn btn-primary " style="border-bottom:2px solid red;" name="reply" > Submit Reply</button>
...
//echo $messageid;
$sql=mysqli_query($conn,"select * from task_reply t inner join employee e on t.reply_by = e.id where message_id='$msgid'");
$numrow=mysqli_num_rows($sql);
while($data=mysqli_fetch_assoc($sql))
{
 ?>
 <div class="col-sm-4" style="color:white; padding:15px;">
<?php
 echo "<b>".$data['e_name']."</b>".'->:'.''.
$data['reply'];
?></div>
<?php
}
